...
namespace TestGenerator
{
    class Generate
    {
        ...
        public static void Initializer()
        {
            ...
            {
                Console.WriteLine("Error fetching or parsin the json file" + e.Message);
            }
        }
            ...
            foreach (var x in json1.Children())
            {
                ...
                foreach (var y in json.Children())
                {
                    ...
                    if (!Base.Equals("/"))
                        p += Base;
                    p += x.Path.ToString();
                    SwaggerOperation temp = new SwaggerOperation(p, y.Path.ToUpper(), t["operationId"].ToString());
                    if (t["responses"] != null)
                    {
                    ...
                    if (t["parameters"] != null)
                    {
                        ...
                        foreach (var param in a.Children())
                        {
                            ...
                            if (!In.Equals("body"))
                                ...
                                type = param["schema"].First.ToString();
                            string item = null;
                            if (param["items"] != null)
                                item = param["items"]["type"].ToString();
        ...
        private static string getFunc(SwaggerOperation op, Record r, List<string> arrays, string code)
        {
            ...
            {
                if (op.resDataType.Equals("integer"))
                    temp += r.res.body + ";\n";
                else
                    temp += "\"" + r.res.body + "\";\n";
            }
